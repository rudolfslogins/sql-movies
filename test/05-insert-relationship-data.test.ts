...
const insertMovieGenres = (
  ...
  return (
    ...
    genres.map(genre => `
    ('${escape(movieId.toString())}',
    '${escape(genreRows.find(it => it.genre === genre)!.id.toString())}'
    )`).join(",")
  );
};
...
const insertMovieActors = (
  ...
  return (
    ...
    actors.map(actor => `
    ('${escape(movieId.toString())}',
    '${escape(actorRows.find(it => it.full_name === actor)!.id.toString())}'
    )`).join(",")
  );
};
...
const insertMovieDirectors = (
  ...
  return (
    ...
    ('${escape(movieId.toString())}',
    '${escape(directorRows.find(it => it.full_name === director)!.id.toString())}'
    )`).join(",")
  );
};
...
const insertMovieKeywords = (
  ...
  return (
    ...
    ('${escape(movieId.toString())}',
    '${escape(keywordRows.find(it => it.keyword === keyword)!.id.toString())}'
    )`).join(",")
  );
};
...
const insertMovieProductionCompanies = (
  ...
  return (
    ...
    ('${escape(movieId.toString())}',
    '${escape(productionCompanyRows.find(it => it.company_name === productionCompany)!.id.toString())}'
    )`).join(",")
  );
};
    ...
    async done => {
      ...
      for (const imdbId of Object.keys(moviesByImdbId)) {
        ...
        if (directors.length > 0) {
          await db.insert(insertMovieDirectors(movieId, directors, directorRows));
        }
      }
